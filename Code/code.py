...
# find an available file name for storing readings
if i <= 25:
    ...
    # create neopixel error if 25 files already exist (storage)
    led[0] = (255, 0, 0)
    break 

while lastreading == 0:
    led[0] = (0, 255, 0)
    try:  # handle OS Errors
        with open(filename, "a") as fp:  # create a file for writing data
            while True:  # forever loop
                timenow = time.monotonic()  # set time variable
                if timenow - start >= 10 and lastreading != 0:  # if no readings in > 10 sec, and some readings have been made, then stop
                    led[0] = (0, 0, 255)
                    break
                if switch.value is False:  # if hall effect sensor is on/True
                    lastreading = time.monotonic()  # get the time now
                    if lastreading - start >= 0.06:  # only take a reading if the car is moving/hall effect sensor is not continuously triggered
                        led[0] = (255, 0, 0)
